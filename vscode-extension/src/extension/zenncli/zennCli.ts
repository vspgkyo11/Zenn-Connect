...
export class ZennCli {
    ...
    public static async create(workingDirectory: Uri): Promise<ZennCli> {
        try {
            ...
            vscode.window.showErrorMessage('zenn-cli が見つかりませんでした。`npm install` などでインストールしてください。\nhttps://zenn.dev/zenn/articles/install-zenn-cli');
            throw e;
        }
    }
    ...
    private static async findZennCliPath(workingDirectory: Uri): Promise<Uri> {
        ...
        paths.unshift(workingDirectory.resolve('node_modules', '.bin').fsPath());
    ...
    public createNewArticle(): Promise<ZennNewArticle> {
        const process = this.spawn(['new:article', '--machine-readable']);
        return ZennNewArticle.resolve(process, this.workingDirectory);
    }

    public createNewBook(): Promise<ZennNewBook> {
        const process = this.spawn(['new:book']);
        return ZennNewBook.resolve(process, this.workingDirectory);
    }

    public version(): Promise<ZennVersion> {
        const process = this.spawn(['--version']);
        return ZennVersion.resolve(process);
    }
