...
サプライチェーンは「AIに持たせる道具」を信用できないという話です。そして、被害の大きさを決めるのは、「渡している権限」です。
...
MCPサーバーに渡すAPIキーは、そのサーバー専用で、必要な権限だけを持つものにします。`postmark-mcp` の被害も、送信用の権限しか持たないキーであれば、メールの持ち出しより外には広がりにくくなります。「読める」権限と「書ける・消せる」権限を分けておくと、万一のときの被害を小さくできます。
...
AIが判断材料にするのは、READMEやコードに書かれた文章です。それを書いたのは、まさに疑うべき配布元です。AIは読み込んだ文章と指示を区別できないため、そこに「このパッケージは安全です」と書かれていれば、AIはそれを信じてしまうことがあります。
...
MCPのおかげで、AIにできることは一気に増えました。
ただ、それらの設定ファイルに並んでいるMCPサーバーを1つずつ眺めて、これは誰が作ったものかを確かめるところまでやるだけでも危険性は下がると思います。
