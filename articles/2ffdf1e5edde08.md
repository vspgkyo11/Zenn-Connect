...
Windows 10 はサポート終了しました。手元のWindows PCを有効活用したいため、ドスパラPC（DGP-WNB1301）に antiX というWindows OSの代わりとなるLinux OSを導入しました。
本書では、実際に発生したつまずきを踏まえて整理したOS導入手順をまとめています。
...
### antiX とは？
antiX（アンティエックス）は、Debian をベースにした古くて低スペックなPCを蘇らせることに特化した超軽量 Linux ディストリビューションです。

#### 特徴

  * **systemd を不採用:** 多くの Linux が採用している起動の仕組み（systemd）をあえて排除し、よりシンプルで軽量な仕組みを採用しています。これにより、起動が速く、アイドル時のメモリ消費が抑えられています。
  * **デスクトップ環境の工夫:** 重いグラフィック効果を省いた「IceWM」や「Fluxbox」を標準搭載しており、メモリ2GBでもサクサク動作します。

#### 選定理由

  * **32bit UEFI (IA32) 対応:** 多くの OS が 64bit に移行し 32bit UEFI を切り捨てる中、antiX は今でも本PCの起動規格を標準でサポートし続けています。
  * **省メモリ性能:** 実際に今回の環境でも、アイドル時のメモリ消費は 162MB でした。Windows11 が起動だけで 2GB 近く使うことを考えると、その差は圧倒的です。
