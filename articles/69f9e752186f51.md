---
title: "【GAS×Clasp】複数のGASプロジェクトを同一リポジトリで管理する方法"
emoji: "🚀"
...
そして、Github 上で１つのリモートリポジトリで複数プロジェクトのソースを管理出来たら便利ですよね。

そんなときは、**Clasp** を利用すると効率的に開発・デプロイできます。

本記事では、**同一リポジトリで複数の GAS プロジェクトを管理する方法**を、具体的な手順とともに紹介します。特に、コマンド実行時のディレクトリの移動や構成が重要になるため、実際に私が苦労したポイントも含めて詳しく説明します。私の実体験から少し細かい部分まで記載していますが、スムーズな開発のために参考になれば幸いです。

## この記事の対象者

- GAS を複数プロジェクト管理したい
- Clasp を使った GAS 開発をしたい
- GAS と GitHub の CI/CD 連携をしたい

## Clasp とは？

[Clasp](https://github.com/google/clasp)（Command Line Apps Script Projects）は、Google が提供する **CLI ツール** で、ローカル環境で GAS を管理・デプロイできます。

## 本記事の管理方法のメリット

1. すべての GAS プロジェクトを一元管理できます。
  Project1/, Project2/ のようにディレクトリを分け、1つのリポジトリ内で複数の GAS プロジェクトをまとめて管理できます。
...
まず、Github 上での複数 GAS プロジェクトを管理するための完成形としては、以下のようなディレクトリ構成となります。
...
```
:::message
この <repository-name> フォルダが、以降のすべての GAS プロジェクトを管理するルートディレクトリになります。 
各 GAS プロジェクト（例: Project1, Project2）はこのフォルダの中に作成されます。
:::
...
3. `.clasprc.json` 内の JSON の "token" の すべての値 をコピーし、GitHub リポジトリ の以下の設定画面において CLASP_TOKEN という名前で保存します。
...
#### 保存する値
Repository secrets に CLASP_TOKEN という名前で次のように保存する
...
:::message
clasp コマンドを実行する際は、`.clasprc.json` と同じディレクトリ内で実行しましょう。
:::
...
✅ **ディレクトリ構成の整理**
✅ **新規・既存の GAS プロジェクトの追加方法**
✅ **GitHub Actions を用いた自動デプロイ**

これで、複数の GAS プロジェクトを効率的に管理できるようになります！🚀
