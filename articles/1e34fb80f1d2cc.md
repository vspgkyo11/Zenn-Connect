...
1. **Application Layer**: Laravel Project Source Code
2. **Framework Layer**: Laravel Core
3. **Middleware Layer**: PHP, Nginx, MySQL, Redis
4. **Infrastructure Layer**: OS (Linux), Docker Container

### 原則：依存の方向性

依存関係は「上位レイヤーが下位レイヤーに依存する」という構造を持ちます。
Laravel（Framework）は特定のPHPバージョン（Middleware）に依存し、PHPはOS（Infrastructure）上のライブラリに依存します。

したがって、更新計画は「上位レイヤーから要件を確認し（Top-down Analysis）、下位レイヤーから更新を確定させる（Bottom-up Execution）」のが鉄則です。

## 更新ワークフロー

安全なアップグレードを実施するための基本的な手順を以下に示します。

### Step 0: 前提環境の確認 (Prerequisites)
...
### Step 1: 要件定義 (Requirement Analysis)
...
* **PHP Version**: 最低要件バージョン
* **Extensions**: 必要なPHP拡張モジュール（BCMath, Ctype, Fileinfoなど）
* **Database**: 対応するデータベースのバージョン

この段階で、現在の環境とのギャップを特定します。

### Step 2: バックアップとロールバック計画 (Backup & Rollback Strategy)

「バックアップを取る」だけでなく、「失敗した時にどう戻すか（リストア手順）」まで確立してから作業に入ります。

* **Codebase**: Gitによる管理。作業前にタグを打つか、作業用ブランチを作成します。
* **Database**: `mysqldump` 等によるダンプ取得。
    * リストア手順: `mysql -u user -p dbname < backup.sql` が通ることを確認。
* **System**: VPSのスナップショットやAMIの作成。これが最強のロールバック手段です。

> **Warning**: ステージング環境での検証なしに本番環境でこれらを実施するのはとてもリスクが高いです。可能な限り、本番と同等の検証環境でリハーサルを行うことを推奨します。

### Step 3: メンテナンスモードへの切り替え (Maintenance Mode)
...
### Step 4: ミドルウェア層の更新 (System Layer Update)
...
### Step 5: アプリケーション層の更新 (Application Layer Update)
