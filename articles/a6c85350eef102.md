...
AWS EC2上のDebian 12 (Bookworm) 環境において、稼働中のサービスを止めることなくバージョンを上げるまでの手順を備忘録として残します。

Debianの標準リポジトリは安定性を重視するため、最新のPHPバージョンが提供されるまでタイムラグがあります。そこで、DebianおよびUbuntuのPHPをメンテナンスしているOndřej Surý氏が提供する `deb.sury.org` リポジトリを利用するのが一般的です。
...
サーバーのミドルウェアをアップデートする作業は、緊張感がありますよね。特にクライアント管理のものの場合はなおさらです。
ただサーバーを管理するうえではセキュアな状態となるようなメンテナンス作業は欠かせません。
