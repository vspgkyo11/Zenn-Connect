---
title: "【Laravel 10→12】段階的アップグレードガイド"
emoji: "🪜"
...
# はじめに
`EOL` となってしまった `Laravel` プロジェクトの更新作業を実施することがありました。
本記事は、Laravel 10 から 12 へと段階的にアップグレードを実施した時の手順を解説します。

## 対象者

* Laravel 10 以前のプロジェクトを保守しているエンジニア
* Laravel 11 や 12 へアップグレードする予定がある方
* ライブラリの依存関係地獄に陥った経験がある方
...
アップグレード時のトラブルの多くは、古い依存関係の残留が原因です。一度 `composer.lock` と `vendor` ディレクトリを削除し、更地に戻すことで解決することがあります。
...
最終確認として、以下のような項目を確認し、問題がなければ移行は完了です。
...
定期的にやってくる大規模なメジャーバージョンアップは、大変な印象を持っている方も多いと思います。私自身も依存関係地獄にハマってしまい、なかなか思ったようにライブラリやフレームワークの更新が進まないことが多々ありました…。もし同じような状況の方がいればぜひ本記事を参考にしていただきたいです。本記事がプロジェクトのアップグレードの一助となれば幸いです。
