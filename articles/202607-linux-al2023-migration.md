---
title: "【Amazon Linux 2023】nginx+vsftpd構成の静的配信EC2をAL2から新規移行した記録"
emoji: "🔄"
...
Amazon LinuxはAWSがEC2向けに提供しているLinuxで、AL2は2026年6月末にサポート終了 (EOL) を迎えます。EOLを過ぎるとセキュリティ更新が提供されなくなるため、まだAL2で動いているサーバーがあれば、後継であるAL2023への移行が必要になります。

本記事では、実際に移行したときの流れをフェーズごとに整理します。細かい設定の話には踏み込まず、「どういう順番で何をやるのか」を掴んでもらうことを目的とした内容です。
...
AL2からAL2023への移行は、in-placeアップグレードが用意されていないため身構えてしまいますが、やること自体は「新しく立てて、設定とデータを移して、切り替える」というシンプルな流れです。特に今回のようなPHPもDBも持たない静的配信サーバーであれば、移行対象も少なく取り組みやすい部類だと思います。

一方で、旧サーバーの設定をそのまま移せば動くとは限らず、AL2とAL2023の細かな差分に足を止められる場面もありました。そのあたりは、また別の記事にまとめられればと思います。
AL2のEOL対応をこれから始める方の、最初の見取り図になれば幸いです。
